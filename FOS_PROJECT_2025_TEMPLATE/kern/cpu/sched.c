	...
	do
	{
		...
		do
		{
			...
			if(next_env != NULL)
			{
				...
				if(next_env->env_status==ENV_FREE)
				{
					kfree((void*)next_env->kstack);
				}
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	if (isSchedMethodPRIRR())
	{
		...
		for(int i=1;i<num_of_ready_queues;i++)
		{
			...
			{
				e->tick_clock++;
				if(e->tick_clock > starvThresh_limit)
				{
					remove_from_queue(&(ProcessQueues.env_ready_queues[i]),e);
					e->priority--;
...
void update_WS_time_stamps()
{
	...
	//Your code is here
	struct Env* p = get_cpu_proc();
	struct WorkingSetElement * el =NULL;
	LIST_FOREACH_SAFE ( el , &(p->page_WS_list) , WorkingSetElement )
	{
		if(!el->empty)
		{
			int perms=pt_get_page_permissions(p->env_page_directory,el->virtual_address);
			int used_bit = (perms & PERM_USED) ? 1 : 0;
			el->time_stamp=((el->time_stamp)>>1)|(used_bit << 31);
			pt_set_page_permissions(p->env_page_directory,el->virtual_address,0,PERM_USED);
		}
	}
#endif
	//Comment the following line
