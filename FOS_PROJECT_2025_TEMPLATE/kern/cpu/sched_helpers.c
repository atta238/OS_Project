...
void sched_kill_env(uint32 envId)
{
	...
	else
	{
		...
		if (cur_env->env_id == envId)
		{
			...
			char * tempo=ptr_env->kstack;
			env_free(ptr_env);
			ptr_env->kstack=tempo;
			cprintf("[END] DONE\n");
			found = 1;
...
void env_set_priority(int envID, int priority)
{
	...
	{
		panic("INCORRECT VALUE FOR PRIORITY YA BASHA");
	}
	struct Env * e;
	envid2env(envID,&e,0);
	if(e!=NULL)
	{
		...
		{
			int last_pri=e->priority;
			e->priority=priority;
			acquire_kspinlock(&(ProcessQueues.qlock));
			if(e->env_status == ENV_READY)
			{
				LIST_REMOVE(&(ProcessQueues.env_ready_queues[last_pri]), e);
	...
	{
		panic("ENV ID NOT FOUND YA HABEBY");
	}
#endif
