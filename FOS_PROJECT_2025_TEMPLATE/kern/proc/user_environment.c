...
struct Env* env_create(char* user_program_name, unsigned int page_WS_size, unsigned int LRU_second_list_size, unsigned int percent_WS_pages_to_remove)
{
	...
	for(int i=0;i<w;i++)
	{
		struct WorkingSetElement *p=env_page_ws_list_create_element(e,e->prepagedVAs[i]);
		LIST_INSERT_TAIL(&(e->Active_WS),p);
	}
...
void env_free(struct Env *e)
{
	...
	struct WorkingSetElement * xcv=NULL;
	LIST_FOREACH_SAFE(xcv,&e->page_WS_list,WorkingSetElement)
	{
		unmap_frame(e->env_page_directory,xcv->virtual_address);
		LIST_REMOVE(&e->page_WS_list,xcv);
		kfree((void*)xcv);
	}
	// [3] free the PAGE working set itself from the main memory
	kfree((void*)e->prepagedVAs);
	struct WorkingSetElement * xcv1=NULL;
	{LIST_FOREACH_SAFE(xcv1,&e->Active_WS,WorkingSetElement)
	{
		unmap_frame(e->env_page_directory,xcv1->virtual_address);
		LIST_REMOVE(&e->Active_WS,xcv1);
		kfree((void*)xcv1);
	}}
	// [4] free the USER HEAP block allocator [if exists]
	uint32 maxi=(USER_HEAP_START + DYN_ALLOC_MAX_SIZE);
	for(uint32 i=USER_HEAP_START;i<maxi;i+=PAGE_SIZE)
	{
	...
	{
		del_page_table(e->env_page_directory,i);
	}
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	#if USE_KHEAP
	//Your code is here
	void * sal_hab=kmalloc(KERNEL_STACK_SIZE);
	//guard page
	pt_set_page_permissions(ptr_user_page_directory,(uint32) sal_hab, 0, PERM_PRESENT);
	return sal_hab;
	#endif
	//Comment the following line
