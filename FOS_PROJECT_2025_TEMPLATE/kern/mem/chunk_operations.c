...
//		return;
	...
	{
		uint32 ptr=virtual_address+(i*PAGE_SIZE);
		get_page_table(e->env_page_directory,ptr,&ptr_table);
		if(ptr_table==NULL)
		{
			ptr_table=create_page_table(e->env_page_directory,ptr);
		}
		pt_set_page_permissions(e->env_page_directory,ptr,PERM_UHPAGE|PERM_USER,0);
	}
#endif
	//Comment the following line
...
//		return;
	...
	for (uint32 i = 0; i < no_pages; i++)
	{
		...
		pt_set_page_permissions(e->env_page_directory,VA,0,PERM_UHPAGE|PERM_USER);
