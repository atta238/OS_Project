...
#if USE_KHEAP
	//Your code is here
	struct Share * awl_wa7ed = (struct Share *) kmalloc(sizeof(struct Share));
	if(awl_wa7ed==NULL)
	{
		return NULL;
	}
	uint32 zoz_id=((uint32)awl_wa7ed)&0x7FFFFFFF;

	awl_wa7ed->ID=zoz_id;
	awl_wa7ed->references=1;
	awl_wa7ed->size=size;
	awl_wa7ed->ownerID=ownerID;
	strcpy(awl_wa7ed->name,shareName);
	awl_wa7ed->isWritable=isWritable;
	uint32 no_pages=ROUNDUP(size,PAGE_SIZE)/PAGE_SIZE;
	awl_wa7ed->framesStorage = (struct FrameInfo**) kmalloc(sizeof(struct FrameInfo*) * no_pages);
	if(awl_wa7ed->framesStorage==NULL)
	{
		kfree((void*)awl_wa7ed);
		return NULL;
	}
	for(int i=0;i<no_pages;i++)
	{
		awl_wa7ed->framesStorage[i]=NULL;
	}
	return awl_wa7ed;
...
int create_shared_object(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	{
		struct Share* awl_wa7ed=alloc_share(ownerID,shareName,size,isWritable);
		if(awl_wa7ed==NULL)
		{
			return E_NO_SHARE;
		}
		...
		for(int i=0;i<no_pages;i++)
		{
			...
			alloc_page(myenv->env_page_directory,VA,PERM_USER|PERM_WRITEABLE|PERM_UHPAGE,0);
			uint32 * ptr_table=NULL;
			awl_wa7ed->framesStorage[i]=get_frame_info(myenv->env_page_directory,VA,&ptr_table);
		}
		bool wasHeld = holding_kspinlock(&(AllShares.shareslock));
		if (!wasHeld)
		{
		...
		LIST_INSERT_TAIL(&AllShares.shares_list,awl_wa7ed);
		int32 id=awl_wa7ed->ID;
		if (!wasHeld)
		{
			release_kspinlock(&(AllShares.shareslock));
		}
...
int get_shared_object(int32 ownerID, char* shareName, void* virtual_address)
{
...
#if USE_KHEAP
	struct Env* myenv = get_cpu_proc(); //The calling environment
	struct Share* awl_wa7ed=find_share(ownerID, shareName);
	if(awl_wa7ed==NULL)
	{
		return E_SHARED_MEM_NOT_EXISTS;
	}
	else
	{
		...
		uint8 read_write=awl_wa7ed->isWritable;
		int hagm=awl_wa7ed->size;
		struct FrameInfo** hnshero=awl_wa7ed->framesStorage;
		awl_wa7ed->references++;
		int32 id=awl_wa7ed->ID;
		uint32 no_pages=ROUNDUP(hagm,PAGE_SIZE)/PAGE_SIZE;
		for(int i=0;i<no_pages;i++)
		{
			uint32 VA=((uint32)virtual_address)+(i*PAGE_SIZE);
			if(read_write)
			{
				map_frame(myenv->env_page_directory,hnshero[i],VA,PERM_USER|PERM_WRITEABLE|PERM_UHPAGE);
			}
			else
			{
				map_frame(myenv->env_page_directory,hnshero[i],VA,PERM_USER|PERM_UHPAGE);
			}
		}
		if (!wasHeld)
		{
